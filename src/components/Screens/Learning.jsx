    ...
    <div className='relative flex flex-col w-full h-screen overflow-hidden'>
      ...
      <div className='relative z-10 flex flex-row items-center h-full'>
        <motion.div variants={move2} initial="hidden" whileInView="show" className='flex flex-col items-start pl-[5%] md:w-3/4'>
          ...
          <motion.h1 variants={move2} className='max-w-5xl p-3 text-xl font-bold text-gray-300'>But I want to be much faster than others</motion.h1>
        </motion.div>
        <h1 className='p-3 text-lg font-bold text-gray-300 absolute bottom-0 left-[5%]'>*All blocks scrolling are technologies I have learned </h1>
        ...
        <ScrollText  speed={-0.7} >
          <div className='flex flex-row gap-4'>
            {skills.map((skill, index) => 
              <div className='bg-[#232a35] rounded-xl p-2 px-3 flex flex-row justify-start items-center w-[400px]'>
                <img src={urlFor(skill.image)} alt="rbc" className='h-[60px] max-w-[80px] object-contain'/>
                <div className='flex flex-col justify-center pl-3'>
                  <h3 className='text-xl font-bold text-white'>{skill.title}</h3>
                  <p className='text-sm font-semibold text-gray-300'>{skill.description}</p>
                </div>
              </div>
            )}
          </div>
